...
from ConfigDB.api.serializers.serializers_netflowassociation \
        import NetFlowAssociation
from ConfigDB.api.serializers.serializers_netflowassociation \
        import NetFlowAssociationSerializer
...
class TestNetFlowAssociationNoRecord(TestCase):
    ...
    def test_save_None_record(self):
        ...
        with self.assertRaises(TypeError):
            NetFlowAssociation().save()

    def test_get_no_record(self):
        """Test case to fetch a NetFlowAssociation with invalid id."""
        with self.assertRaises(ResourceNotFound):
            NetFlowAssociation.get(generate_uuid())

    def test_get_all_no_record(self):
        """Test case to fetch all NetFlowAssociation with no records present"""
        with self.assertRaises(ResourceNotFound):
            NetFlowAssociation.get_all()

    def test_delete_no_record(self):
        """Test case to delete an invalid NetFlowAssociation record."""
        with self.assertRaises(ResourceNotFound):
            NetFlowAssociation().delete()


class TestNetFlowAssociationSerializer(TestCase):
...
#        for serializerdata in serializer_data:
    ...
    def tearDown(self):
        ...
        if self._testMethodName.startswith('test_post_data'):
            record1 = NetFlowAssociation.get(self.uuid1)
            record1.delete()
