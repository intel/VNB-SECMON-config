...
from ConfigDB.api.serializers.serializers_sflowassociation \
        import SflowAssociation
from ConfigDB.api.serializers.serializers_sflowassociation \
        import SflowAssociationSerializer
...
class TestSflowAssociationNoRecord(TestCase):
    ...
    def test_save_None_record(self):
        ...
        with self.assertRaises(TypeError):
            SflowAssociation().save()

    def test_get_no_record(self):
        """Test case to fetch a SflowAssociation with invalid id."""
        with self.assertRaises(ResourceNotFound):
            SflowAssociation.get(generate_uuid())

    def test_get_all_no_record(self):
        """Test case to fetch all SflowAssociation with no records present."""
        with self.assertRaises(ResourceNotFound):
            SflowAssociation.get_all()

    def test_delete_no_record(self):
        """Test case to delete an invalid SflowAssociation record."""
        with self.assertRaises(ResourceNotFound):
            SflowAssociation().delete()


class TestSflowAssociationSerializer(TestCase):
...
#        for serializerdata in serializer_data:
        serializer = SflowAssociationSerializer(data=serializer_data[0])
        if serializer.valid(self.uuid):
            serializer.create(serializer.data)

    def tearDown(self):
        record = SflowAssociation.get(self.uuid)
        record.delete()
        if self._testMethodName.startswith('test_post_data'):
            record1 = SflowAssociation.get(self.uuid1)
            record1.delete()
...
#        for serializerdata1 in serializer_data1:
        serializer = SflowAssociationSerializer(data=serializer_data1[0])
...
#        for serializerdata3 in serializer_data3:
        serializer = SflowAssociationSerializer(data=None)
        if serializer.valid(self.uuid):
            with self.assertRaises(TypeError):
                serializer.create(serializer.data)
#        self.assertEqual(serializerdata1, serializer.data)
    ...
    def test_update_id(self):
        ...
        obj = SflowAssociation.get(self.uuid)
        serializer_data1 = json.loads(json.dumps(self.data1))
#        for serializerdata1 in serializer_data1:
        with self.assertRaises(IDUpdateNotPermitted):
            serializer = SflowAssociationSerializer(
                                        obj,
...
#            with self.assertRaises(TypeError):
#                serializer = SflowAssociationSerializer(obj,
...
#        self.assertEqual(serializer.valid(self.uuid),True)
    ...
    def test_invalid_serializer_with_pk(self):
        ...
        record = SflowAssociation.get(self.uuid)
        serializer = SflowAssociationSerializer(record)
