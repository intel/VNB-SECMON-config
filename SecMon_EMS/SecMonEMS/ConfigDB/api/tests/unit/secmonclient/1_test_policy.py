...
from secmonclient.v1_0.vpn import policy
...
#        pdb.set_trace()
    ...
    def test_add_Argument_with_valid_parser(self):
        ...
        """
#        pdb.set_trace()
        self.subcommand_class = self.commands.get('policy-create')
...
#       args = parser.parse_known_args()
        ...
        self.assertEqual(arg1.ruleobject_id,None)
'''
    def test_add_argument_show(self):
        """test case to test add_argument in show with valid data
        """
        self.subcommand_class = self.commands.get('policy-show')
...
#       args = parser.parse_known_args()
    ...
    def test_add_argument_list(self):
        ...
        """
        self.subcommand_class = self.commands.get('policy-list')
        parser =  argparse.ArgumentParser(description='parser for arguments')
        parser = self.subcommand_class().add_known_arguments(parser)
 #      args = parser.parse_known_args()
        arg1 = parser.parse_args()
...
#       self.assertEqual(arg1.format,None)
    ...
    def test_add_argument_delete(self):
        ...
        """
        self.subcommand_class = self.commands.get('policy-delete')
        parser =  argparse.ArgumentParser(description='parser for arguments')
        parser = self.subcommand_class().add_known_arguments(parser)
 #      args = parser.parse_known_args()
        arg1 = parser.parse_args()
    ...
    def test_add_Argument_with_valid_parser_update(self):
        ...
        """
#        pdb.set_trace()
        self.subcommand_class = self.commands.get('policy-update')
        parser = argparse.ArgumentParser(description='parser for arguments')
        parser = self.subcommand_class().add_known_arguments(parser)
  #     args = parser.parse_known_args()
        arg1 = parser.parse_args()
