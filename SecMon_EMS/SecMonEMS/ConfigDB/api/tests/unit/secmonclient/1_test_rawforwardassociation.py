...
from secmonclient.v1_0.vpn import rawforwardassociation

class TestRawforwardassociation(TestCase):
    def setUp(self):
#        pdb.set_trace()
        self.commands = {
    ...
    def test_rawforwardassociation_create_verify_arguments_with_invalid_data(self):
        """test case to test verify_arguments function with invalid data
        """
        self.subcommand_class = self.commands.get('rawforwardassociation-create')
        with self.assertRaises(argparse.ArgumentTypeError):
            self.subcommand_class().verify_arguments(self.invalid_data)

    def test_rawforwardassociation_update_verify_arguments_with_invalid_data(self):
        """test case to verify verify_arguments for update with invalid data
        """
        self.subcommandupdate_class = self.commands.get('rawforwardassociation-update')
        with self.assertRaises(argparse.ArgumentTypeError):
            self.subcommandupdate_class().verify_arguments(self.invalid_data)

    def test_rawforwardassociation_create_verify_arguments_with_valid_data(self):
    ...
    def test_rawforwardassociation_update_verify_arguments_with_valid_data(self):
...
#        pdb.set_trace()
        self.subcommand_class = self.commands.get('rawforwardassociation-create')
        parser = argparse.ArgumentParser(description='parser for rawforwardassociation arguments')
    ...
    def test_add_argument_show(self):
        ...
        """
        self.subcommand_class = self.commands.get('rawforwardassociation-show')
    ...
    def test_add_argument_list(self):
        ...
        """
        self.subcommand_class = self.commands.get('rawforwardassociation-list')
        parser =  argparse.ArgumentParser(description='parser for arguments')
        parser = self.subcommand_class().add_known_arguments(parser)
        args = parser.parse_known_args()
        arg1 = parser.parse_args()
...
#       self.assertEqual(arg1.format,None)
    ...
    def test_add_argument_delete(self):
        ...
        """
        self.subcommand_class = self.commands.get('rawforwardassociation-delete')
        parser =  argparse.ArgumentParser(description='parser for arguments')
        parser = self.subcommand_class().add_known_arguments(parser)
        args = parser.parse_known_args()
        arg1 = parser.parse_args()
    ...
    def test_add_Argument_with_valid_parser_update(self):
        ...
        """
#        pdb.set_trace()
        self.subcommand_class = self.commands.get('rawforwardassociation-update')
        parser = argparse.ArgumentParser(description='parser for rawforwardassociation arguments')
        parser = self.subcommand_class().add_known_arguments(parser)
        args = parser.parse_known_args()
        arg1 = parser.parse_args()
