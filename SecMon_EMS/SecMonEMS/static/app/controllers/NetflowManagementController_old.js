            ...
            .then(function(response) {
                ...
                $http.get("/v1.0/secmon/collectorset/netflow/")
                    .then(function(response) {
                        ...
                        // $scope.available_netflowcollectorsets[ncsIndex].collectorsIdArray = JSON.parse($scope.available_netflowcollectorsets[ncsIndex].collector_ids.replace(/'/gi, "\""));
                        // $scope.available_netflowcollectorsets[ncsIndex].allCollName = "";

                        // for (var nciIndex = 0; nciIndex < $scope.available_netflowcollectorsets[ncsIndex].collectorsIdArray.length; ++nciIndex) {
                        // for (var ncIndex = 0; ncIndex < $scope.available_netflowcollectors.length; ++ncIndex) {
                        // if ($scope.available_netflowcollectors[ncIndex].id === $scope.available_netflowcollectorsets[ncsIndex].collectorsIdArray[nciIndex].id) {
                        // $scope.available_netflowcollectorsets[ncsIndex].collectorsIdArray[nciIndex].name = $scope.available_netflowcollectorsets[nciIndex].name;
                        // }
                        // }
                        // }

                        // for (var ncIndex = 0; ncIndex < $scope.available_netflowcollectors[ncsIndex].collector_ids.length; ++ncIndex) {
                        // console.log("id ", $scope.available_netflowcollectorsets[ncsIndex].collector_ids[ncIndex]);
...
*/
        ...
        // for (var csIndex = 0; csIndex < netflowCollectorSets.length; ++csIndex) {
        //   netflowCollectorSets[csIndex].collectorIds = JSON.parse(netflowCollectorSets[csIndex].collector_ids.replace(/'/gi, "\""));
        //   var collectorIds = netflowCollectorSets[csIndex].collectorsIds;
        //   for (var cIndex = 0; cIndex < collectorIds.length; ++cIndex) {
        //     for (var acIndex = 0; acIndex < $scope.available_netflowcollectors.length; ++acIndex) {
        //       if (collectorIds[cIndex] === $scope.available_netflowcollectors[acIndex].id) {
        //         console.log($scope.available_netflowcollectors[acIndex].name);
        //       }
        //     }
        //   }
        // }
        ...
        //fetch and populate netflowassociation configurations in netflowassociation data table
        ...
        $mdDialog.show(confirm).then(function() {
            ...
            $http.delete("/v1.0/secmon/collector/" + cred_netflowcollector_id + "/").then(
                ...
                function(response) {
                    ...
                    ErrMgmtService.showErrorMsg("Error: netflowcollector deletion failed.", response);
    ...
    $scope.deleteNetflowConfig = function(ev, cred_netflowconfig_id) {
        ...
        var confirm = $mdDialog.confirm()
            .title('Delete?')
            .textContent("Deleting the netflowconfig " + cred_netflowconfig_id + " would disable all access to it. Are you sure you want to delete the netflowconfig?")
            .targetEvent(ev)
            .ok('Yes, Delete it!')
            .cancel("No");
        ...
        $mdDialog.show(confirm).then(function() {
            ...
            $http.delete("/v1.0/secmon/netflowconfig/" + cred_netflowconfig_id + "/").then(
                ...
                function(response) {
                    $('html,body').scrollTop(0);
                    ErrMgmtService.showErrorMsg("Error: netflowconfig deletion failed.", response);
    ...
    $scope.deleteNetflowMonitor = function(ev, cred_netflowmonitor_id) {
        ...
        var confirm = $mdDialog.confirm()
            .title('Delete?')
            .textContent("Deleting the netflowmonitor " + cred_netflowmonitor_id + " would disable all access to it. Are you sure you want to delete the netflowmonitor?")
            .targetEvent(ev)
            .ok('Yes, Delete it!')
            .cancel("No");
        ...
        $mdDialog.show(confirm).then(function() {
            ...
            $http.delete("/v1.0/secmon/netflowmonitor/" + cred_netflowmonitor_id + "/").then(
                ...
                function(response) {
                    $('html,body').scrollTop(0);
                    ErrMgmtService.showErrorMsg("Error: netflowmonitor deletion failed.", response);
    ...
    $scope.deleteNetflowAssociation = function(ev, cred_netflowassociation_id) {
        ...
        $mdDialog.show(confirm).then(function() {
            ...
            $http.delete("/v1.0/secmon/netflowassociation/" + cred_netflowassociation_id + "/").then(
                ...
                function(response) {
                    $('html,body').scrollTop(0);
                    ErrMgmtService.showErrorMsg("Error: netflowassociation deletion failed.", response);
    ...
    $scope.deleteNetflowCollectorSet = function(ev, cred_netflowcollectorset_id) {
        ...
        $mdDialog.show(confirm).then(function() {
            ...
            $http.delete("/v1.0/secmon/collectorset/" + cred_netflowcollectorset_id + "/").then(
                ...
                function(response) {
                    $('html,body').scrollTop(0);
                    ErrMgmtService.showErrorMsg("Error: netflowcollectorset deletion failed.", response);
...
app.controller('CreateNetflowAssociationController', function($scope, $log, $rootScope, $http, $mdDialog, $mdMedia, ScopeService, ErrMgmtService) {
    ...
    $scope.submit = function() {
        ...
        console.log($scope.collector_id.id)
